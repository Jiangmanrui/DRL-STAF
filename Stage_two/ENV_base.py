...
class Env(gym.Env):
    ...
    def step(self, state, probs, action, prednet, rewardbase=0, pred_sr=None):
        ...
        if done:
            ...
            # print(is_one_states.shape,self.actionhz.shape)
            # print("actionhz", self.actionhz)
    ...
    # def step2(self, state, probs, action, prednet, prednetbase, rewardbase=0):
    #     self.sigsig = 0
    #     # print("env_action",action.shape)
    #     self.con_theta = np.min(
    #         [self.con_theta + (
    #                 np.max([self.con_theta_episode_count - self.con_theta_episode_count_yz, 0]) / 1000) * 0.0005,
    #          self.con_theta_max])
    #     reward2 = 0
    #     if self.last_action < 0:
    #         self.last_action = action
    ...
    #     predbase = prednet.predictbase(state)
    #
    #     predb, pred_kb = prednetbase.predict(state, actiong)
    #     predbaseb = prednetbase.predictbase(state)
    #
    #     self.pred = pred
    #     self.pred_k = pred_k
    #     self.predbase = predbase
    ...
    #             self.errorbase[0, 0] - self.s4[-1, action])) + reward2 - rewardbase
    #
    #     error_kb = (pred_kb - target) ** 2
    #     errorbaseb = (predbaseb - target) ** 2
    #     rewardb = self.score1bs * (
    #             self.future_weight * (errorbaseb[0, 0] - error_kb[0, action]) + (1 - self.future_weight) * (
    #             self.errorbase[0, 0] - self.s4[-1, action])) + reward2 - rewardbase
    #     if rewardb > reward:
    #         self.sigsig = 1
    #     self.errorbase = errorbase
    ...
    #         self.errorhz = np.array(self.errorhz)
    #         self.choicehz = np.array(self.choicehz)
    #         # print(self.actionhz.shape)
    #         # print(self.probhz.shape)
    #         is_one_states = np.argmax(self.probhz, axis=1)
    #         # print(is_one_states.shape,self.actionhz.shape)
    #         # print("actionhz", self.actionhz)
    #
    ...
    #             chaju[chaju < 0] *= self.chajubs
    #             reward_final = -np.average(s_errors_mean) + chaju.sum() - state_ave_chazhi * self.score2bs
    #             reward = reward + reward_final - rewardbase
    #         else:
    #             reward = reward - 100 - rewardbase
    #             print("all actions are the same")
    #         if self.type == 0:
    #             self.com_p[self.current_start - np.max(self.window_size) - 1] = reward
    #             self.sort_p = inverse_softmax(self.com_p)
    #     next_state = self._get_observation()
    #
    #     return next_state, reward, done, pred_train
